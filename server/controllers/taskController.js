...
export const createTask = async (req, res) => {
  try {
    ...
    if (team?.length > 1) {
      text = text.slice(0, -1) + ` and ${team?.length - 1} others.`;
    }

    text =
      text +
      ` The task priority is set to ${priority} priority, so check and act accordingly. The task date is ${new Date().toDateString()}. Thank You!`;
